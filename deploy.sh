...
msg "Pulling down the \`master\` branch into \`docs\` to help avoid merge conflicts"
git subtree pull --prefix=docs \
	git@github.com:$USERNAME/$USERNAME.github.io.git master -m "Merge master"

msg "Building the website"
hugo

msg "Pushing the updated \`docs\` folder to the \`$SOURCE\` branch"
git add docs
git commit -m "$MESSAGE"
git push origin "$SOURCE"

msg "Pushing the updated \`docs\` folder to the \`master\` branch"
git subtree push --prefix=docs \
